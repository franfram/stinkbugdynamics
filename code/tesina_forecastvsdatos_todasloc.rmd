...
for (k in 1:2){  #"1:nLocalidades" 
  for (s in 1){ #"1:length(sample.size)"
    for (r in 1){   #"1:max(repl)"
      ...
      # nos quedamos solo con la posterior de la localidad con sem-menos 
      postdata.V1.EqNv.dat.NOAA <- bind_cols(
        postdata.V1.NOAA %>% 
          select(contains("EqNv.dat") & 
                 ends_with(str_c(k, "]"))), # can't use loc.suffix due to comma
      #also pobs isnt included in the select contains EqNv.dat etc, thus the bind.cols for quick fix
        postdata.V1.NOAA %>% 
      ...
      post.X.A.V1.EqNv.dat.NOAA[[k]] <- postdata.V1.EqNv.dat.NOAA %>%  
        select(starts_with("X.A.EqNv.dat")) #%>%
        #select(ends_with(loc.suffix[j])) # this lines gives an empty tibble
      ...
      ## Temp conocidas) 
      ### en realidad no deberian ser arrays porque pasaste de calcular todas las locs a calcular 
      ### la loc k
      ...
      X.A.F1.EqNv.dat <- l.X.A.F1.EqNv.dat[[k]]
...
for (j in 1:nLocalidades){# 1:nLocalidades
  Y.A.F1.EqNv.dat.mean.planta[[j]] <- 
    apply(Y.A.F1.EqNv.dat.planta[[j]][, -1], 2, mean, na.rm = TRUE) 
  # -1 para sacar la col post.sample
  Y.A.F1.EqNv.dat.median.planta[[j]] <- 
    apply(Y.A.F1.EqNv.dat.planta[[j]][, -1], 2, median, na.rm = TRUE)
  
  Y.A.F1.EqNv.dat.HPDI60.planta[[j]] <- 
    apply(Y.A.F1.EqNv.dat.planta[[j]][, -1], 2, HPDI, prob = 0.60)
  
  Y.A.F1.EqNv.dat.HPDI80.planta[[j]] <- 
    apply(Y.A.F1.EqNv.dat.planta[[j]][, -1], 2, HPDI, prob = 0.80)
  
  Y.A.F1.EqNv.dat.HPDI95.planta[[j]] <- 
    apply(Y.A.F1.EqNv.dat.planta[[j]][, -1], 2, HPDI, prob = 0.95)
...
for (j in seq_along(Localidades)){ # seq_along(Localidades)
  ...
  names(data_plot_obs) <- c("rowid", "semanas_obs", "EqNv_obs", "phenology") # , "threshold"

  data_plot_full[[j]] <- left_join(data_plot_post, data_plot_obs, by = "rowid")  
